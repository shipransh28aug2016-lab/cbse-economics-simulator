...
| I-1 (P0): full teacher-TLM component set absent | ✅ Built — `sim.tlm` (`js/sim-engine.js`'s `renderTLM`/`renderQuickCheck`, new `#tlm-card`) covers 8 of the 10 named artifact types (ConceptCard and InteractiveDiagram already existed): KeyIdea, CommonMistake, ExamTip, ThinkQuestion, Activity, ExitTicket, TeacherExplain, QuickCheck. Mode-agnostic (proven on 3 Graph Labs and 2 simulator-mode sims). Live, full bilingual content, on all 5 gold-standard-order modules. Not yet propagated beyond these 5. |
| D-2 (P1): no misconception engine | 🟡 Partially addressed — a wrong quiz option can now carry its own targeted `misconceptions` text (`js/quiz-engine.js`, keyed by option text so it survives shuffling), shown only when that option is picked; live on 3 of the 5 gold-standard sims' quiz banks (Demand, Supply, PPF). This is a real building block (a specific wrong-answer → specific correction), not yet the full "detect → correct → retry" loop Stage 13 describes (no forced retry after a miss). |
